...
一个用于抖音网页版短视频和直播的 Tampermonkey 播放监测与恢复脚本。它会识别页面已经下发的候选线路、学习线路表现，并在播放器缓冲或画面假死时主动恢复。

## 致谢与来源

本项目的播放恢复思路、面板信息层级和交互方向借鉴了 [realzza/bilibili-accelerator](https://github.com/realzza/bilibili-accelerator)（MIT License），包括在网页环境中观察播放器状态、发生卡顿时优先使用站点下发的备用 CDN 地址，以及以连接状态、速度曲线、主开关和高级设置组织悬浮面板。感谢原作者公开这套思路和实现。完整许可声明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
...
- 监听 `<video>` 的 `waiting`、`stalled`、`playing` 和缓冲区，并独立观察播放时间轴；即使网页没有触发缓冲事件，画面长时间不前进也会识别为假死。
- 默认持续卡顿 4.2 秒后，尝试切到页面原本提供的另一个地址。
- 普通视频时间轴约 2.8 秒不前进、直播约 5.5 秒不前进时会直接进入恢复，不必继续等待网页自己的卡顿事件。
- 直播先跳到最新缓冲、唤醒播放器并临时允许下一次取流改用同清晰度、同协议的页面候选；仍未恢复时才点击站点重试控件。自动刷新直播页默认关闭；手动开启后仍有保护：至少间隔 45 秒，10 分钟内最多两次。
- 关闭“启用监测”后不会继续重排候选或改写直播请求。
...
- **已启动，尚未介入**：播放器正常，脚本已经接管监测但暂时没有必要改变播放行为。
...
- 面板中的曲线优先显示媒体请求的实时下载速度；浏览器没有提供传输大小时退化为前方缓冲曲线。
